...
async def compact_conversation(
    messages: List[BaseMessage], target_tokens: int = 10000
) -> List[BaseMessage]:
    """
    ...
    summary_prompt = (
        get_prompt("compaction.summarize")
...
*Recent conversation continues below...*"""
        ...
        print(
            f"[COMPACTION] Reduced {original_tokens:,} → {new_tokens:,} tokens (saved {original_tokens - new_tokens:,})"
        )
