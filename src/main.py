...
def parse_args():
    ...
    parser.add_argument(
        "--cli", action="store_true", help="Run in CLI mode instead of TUI"
    )
    ...
    # Headless mode
    parser.add_argument(
        "-p", "--prompt", help="Execute single prompt and exit (headless mode)"
    )

    # Agent and skill selection
    parser.add_argument("--agent", help="Specify agent to use (e.g., web-researcher)")
    ...
    if args.prompt:
        ...
        async def headless_execute():
            ...
            try:
                ...
                await run_agent(
                    args.prompt, agent_name=args.agent, skill_names=args.skills or []
                )
                return 0  # Success
            except Exception as e:
            ...
            while True:
                try:
                    ...
                    if not user_input.strip():
                        continue
                    await run_agent(
                        user_input, agent_name=args.agent, skill_names=args.skills or []
                    )
