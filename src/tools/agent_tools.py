...
@tool
async def spawn_agent(agent_name: str, task: str) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        instance_id = await manager.spawn_agent(
            agent_name=agent_name, task=task, skill_names=agent_config.required_skills
        )

        skills_msg = (
            ", ".join(agent_config.required_skills)
            if agent_config.required_skills
            else "none"
        )
        ...
        return {"status": "error", "message": f"❌ Failed to spawn agent: {str(e)}"}
...
async def get_agent_result(instance_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if agent.state == "completed":
            ...
            return {**base_result, "message": "🔄 Agent is initializing..."}
...
def list_available_agents() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"agents": agents, "count": len(agents), "message": message}
    except Exception as e:
        return {
            "agents": [],
            "count": 0,
