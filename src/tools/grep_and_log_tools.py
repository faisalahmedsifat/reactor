    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            try:
                ...
                if isinstance(log_entry, dict) and any(k in log_entry for k in ['level', 'severity', 'message', 'msg']):
                    level = str(log_entry.get('level') or log_entry.get('severity', 'UNKNOWN')).upper()
            ...
            if not matched and current_entry and include_stack_traces:
                ...
                if any(indicator in line for indicator in ['  File "', 'Traceback', 'at ', 'Exception', 'Error:']):
        ...
        stats = {level: len(entries) for level, entries in entries_by_level.items() if entries}
...
async def tail_file(file_path: str, num_lines: int = 10, follow: bool = False) -> Dict:
    ...
    try:
        ...
        return {
            ...
            "line_numbers": list(range(len(lines) - len(tail_lines) + 1, len(lines) + 1))
        }
    ...
    try:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if re.search(error_pattern, line, re.IGNORECASE):
                ...
                current_error = {
                    ...
                    "context_before": [l.strip() for l in lines[max(0, line_num-3):line_num-1]],
                }
            ...
            # Collect stack trace lines
                ...
                if any(indicator in line for indicator in ['  File "', '    at ', '^', 'line ']):
        ...
        if group_similar:
            for error in errors:
                ...
                error_line = re.sub(r'0x[0-9a-fA-F]+', '0xHEX', error_line)  # Replace hex
