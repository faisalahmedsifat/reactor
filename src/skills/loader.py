...
class SkillLoader:
            ...
            if skill_path.exists() and skill_path.is_dir():
                for md_file in skill_path.glob("*.md"):
                    try:
                        ...
                        if skill.name not in cls._cache or str(md_file).startswith(
                            os.getcwd()
                        ):
    ...
    def _load_skill_file(cls, file_path: Path) -> SkillConfig:
        ...
        if "name" not in frontmatter:
            raise ValueError(
                f"Skill file {file_path} missing 'name' field in frontmatter"
            )
        if "description" not in frontmatter:
            raise ValueError(
                f"Skill file {file_path} missing 'description' field in frontmatter"
            )
