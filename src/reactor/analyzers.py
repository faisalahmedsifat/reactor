...
class DependencyAnalyzer:
    ...
    async def find_dependents(self, file_path: str) -> List[str]:
        ...
        try:
            ...
            for candidate_file in project_files:
                ...
                imports = await self.ast_manager.extract_imports_from_file(
                    candidate_file
                )
    ...
    def _is_import_of_module(
        self, import_name: str, module_name: str, file_path: str
    ) -> bool:
    ...
    async def _resolve_import_to_file(
        self, import_name: str, source_file: str
    ) -> Optional[str]:
...
class ImpactAnalyzer:
    ...
    async def assess_impact(
        ...
        try:
            ...
            # Categorize impact
            impact_level = self._categorize_impact(
                breaking_changes, api_changes, affected_files
            )
            ...
            if breaking_changes:
                result.warnings.append(
                    f"Breaking changes detected: {len(breaking_changes)} items"
                )

            if affected_files:
                result.warnings.append(
                    f"Changes affect {len(affected_files)} dependent files"
                )
    ...
    def _function_signature_changed(
    ...
    def _categorize_impact(
        self, breaking_changes: List, api_changes: List, affected_files: List
    ) -> str:
