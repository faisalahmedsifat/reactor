...
class FeedbackFormatter:
    ...
    def format_feedback(
        ...
        # Build summary
        feedback["summary"] = self._build_summary(
    ...
    def _format_auto_fixes(self, auto_fix_result) -> Dict[str, Any]:
        ...
        auto_fixes = {"applied": [], "failed": [], "count": 0}
    ...
    def _build_summary(
        self, validation_result, impact_result, auto_fix_result
    ) -> Dict[str, Any]:
        ...
        # Add impact level
        if hasattr(impact_result, "details") and impact_result.details:
            summary["impact_level"] = impact_result.details.get(
                "impact_level", "minimal"
            )
        ...
        elif validation_result.warnings or (
            hasattr(impact_result, "warnings") and impact_result.warnings
        ):
    ...
    def _generate_suggestions(
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                if validation_result.line_number:
                    suggestions.append(
                        f"Check line {validation_result.line_number} for syntax issues"
                    )
            ...
            if hasattr(impact_result, "details") and impact_result.details:
                ...
                if breaking_changes:
                    suggestions.append(
                        f"Review {len(breaking_changes)} breaking change(s)"
                    )
                ...
                if len(affected_files) > 0:
                    suggestions.append(
                        f"Check {len(affected_files)} affected file(s) for compatibility"
                    )
                ...
                if api_changes:
                    suggestions.append(
                        "Consider updating documentation for API changes"
                    )

            # Auto-fix suggestions
            if (
                hasattr(auto_fix_result, "fixes_failed")
                and auto_fix_result.fixes_failed
            ):
            ...
            if self.config.get_verbosity() == "detailed":
                if validation_result.is_valid and not impact_result.details.get(
                    "breaking_changes"
                ):
    ...
    def format_minimal_feedback(
        self, file_path: str, validation_result, impact_result
    ) -> Dict[str, Any]:
        ...
        if hasattr(impact_result, "details") and impact_result.details:
            ...
            if affected_count > 0:
                feedback["impact"] = {"affected_files": affected_count}
    ...
    def format_error_feedback(
        self, file_path: str, error_message: str, operation: str = "write"
    ) -> Dict[str, Any]:
...
class ProgressFormatter:
    ...
    def format_progress(
        self, operation: str, current: int, total: int, details: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def format_batch_summary(
        self, operation: str, results: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
