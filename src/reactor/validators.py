    ...
    async def validate_syntax(self, file_path: str, content: str) -> ValidationResult:
        ...
        try:
            ...
            if len(content.encode('utf-8')) > self.config.get_max_file_size_mb() * 1024 * 1024:
                ...
                result.errors.append(f"File too large: exceeds {self.config.get_max_file_size_mb()}MB limit")
                return result
        ...
        if result.is_valid and (result.language == Language.JAVASCRIPT or result.language == Language.TYPESCRIPT):
...
class ImportValidator:
    ...
    async def validate_imports(self, file_path: str, content: str) -> ValidationResult:
        ...
        try:
            ...
            for import_name in imports:
                validation_result = await self._validate_single_import(import_name, file_path)
            ...
            if invalid_imports:
                result.is_valid = False
                result.errors.extend([f"Invalid import: {imp}" for imp in invalid_imports])
            
            if missing_modules:
                result.warnings.extend([f"Missing module: {imp}" for imp in missing_modules])
    ...
    async def _validate_single_import(self, import_name: str, file_path: str) -> Tuple[bool, bool]:
        """
    ...
    async def _validate_relative_import(self, import_name: str, file_path: str) -> Tuple[bool, bool]:
...
class CompositeValidator:
    ...
    def __init__(self, ast_manager: ASTManager, config: ReactorConfig):
        self.config = config
        self.syntax_validator = SyntaxValidator(ast_manager, config) if config.should_validate_syntax() else None
        self.import_validator = ImportValidator(ast_manager, config) if config.should_validate_imports() else None
    ...
    async def validate(self, file_path: str, content: str) -> ValidationResult:
        ...
        if self.syntax_validator:
            syntax_result = await self.syntax_validator.validate_syntax(file_path, content)
        ...
        if self.import_validator:
            import_result = await self.import_validator.validate_imports(file_path, content)
