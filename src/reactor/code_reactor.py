...
class CodeReactor:
    ...
    def __init__(
        ...
        ast_manager: Optional[ASTManager] = None,
    ):
    ...
    async def validate_file(self, file_path: str, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "success",
                "file": file_path,
                "validation": self.feedback_formatter._format_validation(
                    validation_result
                ),
    ...
    async def get_project_analysis(self, file_paths: List[str]) -> Dict[str, Any]:
        ...
        """
        try:
            dependency_graph = await self.dependency_analyzer.analyze_dependency_graph(
                file_paths
            )
            ...
            files_with_dependencies = sum(
                ...
                if info.get("dependency_count", 0) > 0
            )
            files_with_dependents = sum(
                1
                for info in dependency_graph.values()
                if info.get("dependent_count", 0) > 0
            )
            ...
            return {
                ...
                "project_stats": {
                    ...
                    "dependency_ratio": (
                        files_with_dependencies / total_files if total_files > 0 else 0
                    ),
                },
            ...
            return {"status": "error", "error": f"Project analysis error: {str(e)}"}
