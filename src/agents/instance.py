...
class AgentInstance:
    ...
    def __init__(self, agent_name: str, task: str, skill_names: List[str] = None):
        ...
        self._message_callbacks = (
            []
        )  # List of async callbacks: (agent_id, node_name, message)
    ...
    async def _handle_stream_events(self, input_state: dict):
        ...
        async for event in self.graph.astream(
            input_state, self.config, stream_mode="updates"
        ):
    ...
    async def _run_loop(self):
        ...
        try:
            # Initialize state similar to run_agent in graph.py
            initial_state = {
