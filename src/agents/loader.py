...
class AgentLoader:
            ...
            if agent_path.exists() and agent_path.is_dir():
                for md_file in agent_path.glob("*.md"):
                    try:
                        ...
                        if agent.name not in cls._cache or str(md_file).startswith(os.getcwd()):
    ...
    def _load_agent_file(cls, file_path: Path) -> AgentConfig:
        ...
        if "name" not in frontmatter:
            raise ValueError(f"Agent file {file_path} missing 'name' field in frontmatter")
        if "description" not in frontmatter:
            raise ValueError(f"Agent file {file_path} missing 'description' field in frontmatter")
