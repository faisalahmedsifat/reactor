...
class JavaScriptASTParser(BaseParser):
        ...
        if TREE_SITTER_AVAILABLE:
            try:
                ...
                if TREE_SITTER_JS_AVAILABLE:
                    self.js_language = Language(tree_sitter_javascript.language(), 'javascript')
                
                # Initialize TypeScript language
                if TREE_SITTER_TS_AVAILABLE:
                    self.ts_language = Language(tree_sitter_typescript.language_typescript(), 'typescript')
    ...
    async def _parse_with_tree_sitter(self, file_path: str, content: str) -> ASTResult:
        ...
        try:
            ...
            is_typescript = (file_path.endswith('.ts') or 
                            file_path.endswith('.tsx') or
                            'interface ' in content or
                            'type ' in content and '=' in content)
    ...
    def _parse_fallback(self, file_path: str, content: str) -> ASTResult:
        ...
        try:
            ...
            actual_language = Language.TYPESCRIPT if is_typescript else Language.JAVASCRIPT
        ...
        try:
            ...
            for child in node.children:
                if child.type == 'type_annotation':
                    return_type = content[child.start_byte:child.end_byte].replace(':', '').strip()
                    break
        ...
        try:
            ...
            for child in node.children:
                if child.type == 'identifier':
                    ...
                    param_type = content[child.start_byte:child.end_byte].replace(':', '').strip()
                ...
                if child.type == 'class_heritage':
                    for heritage_child in child.children:
                        if heritage_child.type == 'identifier':
                            base_name = content[heritage_child.start_byte:heritage_child.end_byte]
    ...
    def _extract_class_methods_tree_sitter(self, node, content: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for child in node.children:
                if child.type == 'identifier':
                    ...
                    var_type = content[child.start_byte:child.end_byte].replace(':', '').strip()
    ...
    def _calculate_complexity_tree_sitter(self, node) -> int:
        ...
        def traverse(n):
            nonlocal complexity
            if n.type in ['if_statement', 'for_statement', 'while_statement', 'switch_statement', 'try_statement', 'catch_clause']:
                complexity += 1
