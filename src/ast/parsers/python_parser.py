...
class PythonParser(BaseParser):
        ...
        for node in ast.walk(ast_root):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                ...
                for arg in node.args.args:
                    param_type = self._get_type_annotation(arg.annotation) if arg.annotation else ""
                ...
                # Handle *args
                if node.args.vararg:
                    vararg_type = self._get_type_annotation(node.args.vararg.annotation) if node.args.vararg.annotation else ""
                    parameters.append(Parameter(
                ...
                # Handle **kwargs
                if node.args.kwarg:
                    kwarg_type = self._get_type_annotation(node.args.kwarg.annotation) if node.args.kwarg.annotation else ""
                    parameters.append(Parameter(
                ...
                for item in node.body:
                    if isinstance(item, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if self._is_property(item):
                            prop_type = self._get_type_annotation(item.annotation) if item.annotation else ""
            ...
            if isinstance(node, ast.Import):
                ...
                for alias in node.names:
                    import_obj = Import(
                        ...
                        is_standard_library=self._is_standard_library(module) if module else False
                    )
                    imports.append(import_obj)
            ...
            if isinstance(node, ast.Assign):
                ...
                if self._is_module_level(node, ast_root):
                    if isinstance(node.target, ast.Name):
                        var_type = self._get_type_annotation(node.annotation) if node.annotation else ""
                        variable = Variable(
                            name=node.target.id,
                            type_hint=var_type,
                            default_value=None,
    ...
    def _get_type_annotation(self, annotation: ast.AST) -> str:
        ...
        if isinstance(annotation, ast.Name):
            ...
            if sys.version_info >= (3, 9):
                ...
                slice_val = self._get_type_annotation(annotation.slice.value if hasattr(annotation.slice, 'value') else annotation.slice)
    ...
    def _is_method(self, node: Union[ast.FunctionDef, ast.AsyncFunctionDef], tree: ast.AST) -> tuple[bool, Optional[str]]:
    ...
    def _extract_method_info(self, node: Union[ast.FunctionDef, ast.AsyncFunctionDef]) -> Method:
    ...
    def _is_standard_library(self, module_name: str) -> bool:
        ...
        stdlib_modules = {
            ...
            'urllib', 'http', 'socket', 'ssl', 'hashlib', 'hmac', 'secrets',
            'threading', 'multiprocessing', 'concurrent', 'asyncio',
            'logging', 'unittest', 'argparse', 'configparser', 'tempfile',
            'shutil', 'glob', 'fnmatch', 'pickle', 'csv', 'sqlite3'
        }
