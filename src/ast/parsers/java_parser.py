...
class JavaASTParser(BaseParser):
    ...
    def _extract_class_methods(
        self, class_content: str, class_line: int
    ) -> List[Dict[str, Any]]:
        ...
        for match in re.finditer(method_pattern, class_content):
            ...
            if not full_match.strip().endswith(
                ";"
            ):  # Not an abstract method without body
            ...
            if method_body_start != -1:
                ...
                for i, char in enumerate(
                    class_content[method_body_start:], method_body_start
                ):
                    if char == "{":
                        brace_count += 1
                    elif char == "}":
                ...
                if param_str:
                    for param in param_str.split(","):
                        ...
                        if param:
                            ...
                            parameters.append(
                                Parameter(name=param_name, type_hint=param_type)
                            )
