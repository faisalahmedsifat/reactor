...
class ASTCache:
    ...
    def put(
        self, file_path: str, file_hash: str, ast_result: ASTResult, parse_time_ms: int
    ) -> None:
        """Cache AST result."""
        cache_key = self._get_cache_key(file_path, file_hash)
    ...
    def _update_hit_rate(self) -> None:
        ...
        self.metrics.cache_hit_rate = (
            (self.metrics.cache_hits / total * 100) if total > 0 else 0.0
        )
    ...
    def _update_average_parse_time(self, parse_time_ms: int) -> None:
        ...
        if self.metrics.total_parses == 1:
            ...
            self.metrics.average_parse_time = (
                alpha * parse_time_ms + (1 - alpha) * self.metrics.average_parse_time
            )
...
class IncrementalParser:
    ...
    def _update_incremental(
        self, file_path: str, content: str, parser_func
    ) -> ASTResult:
    ...
    def _update_file_state(
        self, file_path: str, content: str, ast_result: ASTResult
    ) -> None:
...
class PerformanceOptimizer:
    ...
    def optimize_parse(self, file_path: str, content: str, parser_func) -> ASTResult:
        ...
        return self.incremental_parser.parse_incremental(
            file_path, content, parser_func
        )

    def optimize_batch_parse(
        self, file_paths: List[str], parser_func
    ) -> List[ASTResult]:
                ...
                for file_path in batch:
                    try:
                        ...
                        future = executor.submit(
                            self.optimize_parse, file_path, content, parser_func
                        )
    ...
    def precompute_common_patterns(self, file_paths: List[str]) -> None:
        ...
        sorted_files = sorted(
            file_access_counts.items(), key=lambda x: x[1], reverse=True
        )
    ...
    def _generate_performance_recommendations(
        ...
        if metrics.cache_hit_rate < 50:
            ...
            recommendations.append(
                "Moderate cache hit rate. Review cache invalidation strategy."
            )

        # Memory usage recommendations
        if metrics.memory_usage_mb > 200:
            recommendations.append(
                "High memory usage. Consider reducing cache size or implementing cache compression."
            )

        # Parse time recommendations
        if metrics.average_parse_time > 1000:  # 1 second
            recommendations.append(
                "Slow parse times. Consider optimizing parsers or using incremental parsing."
            )

        # Incremental update recommendations
        if metrics.incremental_updates < metrics.total_parses * 0.3:
            recommendations.append(
                "Low incremental update usage. Review file change detection logic."
            )
    ...
    def load_cache_state(self, file_path: str) -> bool:
        ...
        try:
            ...
            self.cache.metrics.average_parse_time = metrics_data.get(
                "average_parse_time", 0.0
            )
            self.cache.metrics.incremental_updates = metrics_data.get(
                "incremental_updates", 0
            )
