...
class ArchitectureAnalyzer:
    ...
    def analyze(
        self, ast_result: ASTResult, file_path: str, content: str
    ) -> Dict[str, Any]:
    ...
    def _analyze_coupling(
        self, ast_result: ASTResult, file_path: str
        ...
        for cls in ast_result.classes:
            ...
            concrete_deps = [
                ...
                if not self._is_interface_or_abstract(dep, ast_result.classes)
            ]
        ...
        for cls in ast_result.classes:
            ...
            for method in cls.methods:
                ...
                if any(
                    ...
                    for keyword in ["get", "set", "is", "has"]
                ):
                    method_types.add("accessor")
                elif any(
                    keyword in method.name.lower()
                    for keyword in ["create", "build", "make"]
                ):
                    ...
                    keyword in method.name.lower()
                    for keyword in ["save", "load", "read", "write"]
                ):
                    method_types.add("persistence")
                elif any(
                    keyword in method.name.lower()
                    for keyword in ["render", "draw", "display"]
                ):
                    method_types.add("ui")
                else:
        ...
        if ast_result.imports and ast_result.classes:
            ...
            for class_name, imports in imports_by_class.items():
                for imp in imports:
                    ...
                    if imp_class in imports_by_class and class_name in [
                        i.split(".")[-1] for i in imports_by_class[imp_class]
                    ]:
                        issues.append(
                            ArchitectureIssue(
                                severity="high",
    ...
    def _matches_pattern(self, cls: Class, pattern_info: Dict) -> bool:
        ...
        keyword_matches = sum(
            1 for keyword in pattern_info["keywords"] if keyword in class_text
        )
    ...
    def _calculate_architecture_score(
        self, issues: List[ArchitectureIssue], metrics: ArchitectureMetrics
    ) -> Dict[str, Any]:
    ...
    def _get_architecture_recommendations(
        ...
        if critical_issues:
            recommendations.append(
                "Address critical and high severity architecture issues first"
            )

        # Coupling recommendations
        if metrics.coupling_score < 70:
            recommendations.append(
                "Reduce coupling by implementing dependency injection and using interfaces"
            )

        # Cohesion recommendations
        if metrics.cohesion_score < 70:
            recommendations.append(
                "Improve cohesion by ensuring classes have single responsibilities"
            )

        # Structure recommendations
        if metrics.god_classes > 0:
            recommendations.append(
                "Refactor god classes into smaller, more focused classes"
            )

        # Pattern recommendations
        if metrics.distance_from_main > 0.3:
            recommendations.append(
                "Balance abstractness and instability for better architecture"
            )
    ...
    def _count_class_dependencies(self, cls: Class, ast_result: ASTResult) -> int:
        ...
        for method in cls.methods:
            for param in method.parameters:
                param_type = param.type_hint
                if ast_result.classes and self._is_custom_type(
                    param_type, ast_result.classes
                ):
