...
Analyze and output JSON:"""
    ...
    try:
        ...
        state["messages"].append(
            AIMessage(
                content=f"✓ Understood: {intent_data['task_description']}\nCategory: {intent_data['category']}"
            )
        )
        ...
        state["messages"].append(
            AIMessage(
                content=f"⚠️ Could not parse intent (error: {type(e).__name__}). Using fallback interpretation."
            )
        )
...
Generate execution plan as JSON:"""
    ...
    try:
        ...
        state["messages"].append(
            AIMessage(
                content=f"❌ Could not generate execution plan (error: {type(e).__name__}).\nLLM Response: {response.content[:200]}"
            )
        )
...
"""
        ...
        state["messages"].append(
            AIMessage(
                content=f"""🔍 Error Analysis (fallback):
...
Provide the final answer and reflection:"""
    ...
    state["messages"].append(AIMessage(content=f"💭 Reflection:\n{response.content}"))
