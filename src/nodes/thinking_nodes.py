...
class BrainOutput(BaseModel):
    ...
    analysis: str = Field(
        description="RCA-focused analysis of current state and tool outputs. Why did the last command succeed or fail?"
    )
    plan: str = Field(
        description="High-level view of the workflow to fix or advance. If >2 steps, plan to use create_todo."
    )
    next_step: str = Field(
        description="Specific, executable instruction for the Agent (e.g., 'Run ls -la', 'Read src/main.py')."
    )
    is_terminal: bool = Field(
        default=False, description="True if the user's objective is fully met."
    )
...
async def thinking_node(state: ShellAgentState) -> dict:
    ...
    if len(messages_list) > 0 and isinstance(messages_list[-1], ToolMessage):
        ...
        context_messages.append(
            SystemMessage(
                content="Analyze the request. What is the first step? Output in JSON."
            )
        )
    ...
    if brain_decision.is_terminal:
        ...
        if len(messages_list) > 0 and isinstance(messages_list[-1], ToolMessage):
            ...
            if last_tool.name == "validate_command_safety":
                ...
                if (
                    '"is_safe": true' in content
                    or "'is_safe': True" in content
                    or (
                        last_tool.artifact and last_tool.artifact.get("is_safe") is True
                    )
                ):
    ...
    if not next_step_str or next_step_str.strip() == "":
        if not brain_decision.is_terminal:
            ...
            instruction = (
                "Continue with the next logical step based on the current context."
            )
        else:
    ...
    is_generic_instruction = instruction in [
        "continue_analysis",
        "execute",
        "Continue with the next logical step based on the current context.",
    ]
    ...
    if (
        "analyze" in user_input.lower() or "project" in user_input.lower()
    ) and is_generic_instruction:
